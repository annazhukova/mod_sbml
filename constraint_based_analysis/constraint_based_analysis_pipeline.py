...
def analyse_model(sbml, out_r_id, out_rev, in_r_id2rev_2threshold, res_dir, constraint_cobra_reactions=None,
    ...
    if do_efm:
        ...
        perform_efma(out_r_id, out_rev, out_r_id2rev_2threshold=in_r_id2rev_2threshold, sbml=sbml, directory=efm_dir,
                     max_efm_number=max_efm_number, threshold=threshold,
                     output_efm_file=os.path.join(efm_dir, 'efms.txt'), min_pattern_len=min_pattern_len,
                     min_efm_num_per_pattern=min_efm_num_per_pattern, convert_efms2sbml=save_efm_sbml,
                     convert_patterns2sbml=save_pattern_sbml)
